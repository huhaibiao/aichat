...
const submit = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      //@ts-ignore
      if (ruleForm.username === 'huhaibiao' && ruleForm.password === '1') {
        return loginHandle({ success: true, data: { token: 1, expiresIn: 200000 } })
      } else {
        return loginHandle({ success: false, msg: '登陆失败', data: { token: 1, expiresIn: 200000 } })
      }
...
<template>
  <div class="login-box">
    <div class="login-title">ai chat登录</div>
